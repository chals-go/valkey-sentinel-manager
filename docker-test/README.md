...
```mermaid
graph TD
    subgraph "Valkey Replication Groups"
        V7["valkey7-primary<br/>+ replica x2<br/>(v7.2)"]
        V8["valkey8-primary<br/>+ replica x1<br/>(v8.0)"]
        V8X["valkey8x-primary<br/>+ replica x1<br/>(v8.1)"]
    end

    subgraph "Sentinel Cluster A"
        SA["sentinel-a1 / a2 / a3<br/>+ sentinel-agent<br/>monitors: mymaster-v7"]
    end

    subgraph "Sentinel Cluster B"
        SB["sentinel-b1 / b2 / b3<br/>+ sentinel-agent<br/>monitors: mymaster-v8, mymaster-v8x"]
    end
    ...
    SA -->|"POST /api/v1/events"| SM
    SB -->|"POST /api/v1/events"| SM

    SM["Sentinel Manager<br/>:8000"]
    SM -->|"DNS Update"| MDNS["Mock DNS<br/>REST API :8080"]
    SM -->|"config/events"| STORE["Store Valkey<br/>(data store)"]

    style SM fill:#4f46e5,color:#fff
    style STORE fill:#059669,color:#fff
    style MDNS fill:#f59e0b,color:#fff
    style V7 fill:#3b82f6,color:#fff
    style V8 fill:#3b82f6,color:#fff
    style V8X fill:#3b82f6,color:#fff
```
...
graph TD
    ...
    subgraph "Sentinel Cluster A"
        SA["sentinel-a1 / a2 / a3<br/>+ sentinel-agent<br/>모니터링: mymaster-v7"]
    end

    subgraph "Sentinel Cluster B"
        SB["sentinel-b1 / b2 / b3<br/>+ sentinel-agent<br/>모니터링: mymaster-v8, mymaster-v8x"]
    end
    ...
    SM["Sentinel Manager<br/>:8000"]
    SM -->|"DNS Update"| MDNS["Mock DNS<br/>REST API :8080"]
    SM -->|"config/events"| STORE["Store Valkey<br/>(데이터 저장소)"]
